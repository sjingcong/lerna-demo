...
const currentModuleIndex = computed(() => {
  ...
  return store.modules.findIndex(module => module.moduleCode === store.currentModule?.moduleCode)
})

// 检查是否有下一个模块
const hasNextModule = computed(() => {
  return currentModuleIndex.value >= 0 && currentModuleIndex.value < store.modules.length - 1
})
