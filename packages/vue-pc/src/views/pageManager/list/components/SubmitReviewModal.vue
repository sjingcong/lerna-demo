<template>
  ...
  >
    ...
    >
      <a-form-item label="销售商品名称" name="productName">
        <a-input
      ...
      <a-form-item label="备注标题" name="remarkTitle">
        <a-input
      ...
      <a-form-item label="添加UM账号">
        <div class="um-account-section">
          ...
          />
          <a-button type="primary" @click="handleAddUmAccount">
            添加
          </a-button>
        </div>
      ...
      <a-form-item label="审批链">
        <div class="approval-chain">
          <div v-if="approvalGroups.length === 0" class="empty-chain">
            暂无审批人
          </div>
          <div v-else class="approval-chain-list">
            <template v-for="(group, groupIndex) in approvalGroups" :key="'group-' + groupIndex">
              <div class="approval-group">
                <template v-for="(user, userIndex) in group.users" :key="'user-' + groupIndex + '-' + userIndex">
                  <a-tag
                  ...
                  <span v-if="userIndex < group.users.length - 1" class="group-separator">
                    {{ getSeparatorText(group.groupType) }}
                  </span>
                </template>
              </div>
              <span v-if="groupIndex < approvalGroups.length - 1" class="group-divider">→</span>
            </template>
          </div>
        </div>
      </a-form-item>

      <a-form-item label="审批正文" name="approvalContent">
        <a-textarea
...
const handleAddUmAccount = () => {
  ...
  if (selectedApprovalType.value === 'sequential') {
    ...
    if (!lastGroup) {
      ...
      const typeLabel = approvalTypes.find(t => t.key === selectedApprovalType.value)?.label;
      message.success(`已添加UM账号: ${userName} (新的${typeLabel}组)`);
    } else {
      // 添加到最后一个组
      lastGroup.users.push(userName);
      const typeLabel = approvalTypes.find(t => t.key === lastGroup.groupType)?.label;
...
const handleSubmit = async () => {
  try {
    ...
    const totalUsers = approvalGroups.value.reduce((sum, group) => sum + group.users.length, 0);
    if (totalUsers === 0) {
