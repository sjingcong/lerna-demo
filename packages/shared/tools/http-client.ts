...
class HttpRequest {
  ...
  private handleSuccessResponse(
    ...
    // Token过期处理 - 加入队列等待刷新
    if (
      responseData.code === RESPONSE_CODES.TOKEN_EXPIRED &&
    ...
    if (responseData.code === RESPONSE_CODES.SUCCESS) {
      ...
      const errorMsg =
        responseData.message || errorMsgHandler(Number(responseData.code));
  ...
  public get<T = any>(
    ...
    if (this.isRefreshing && !this.isRefreshTokenRequest(url)) {
      this.logger.info(
        `Token refresh in progress, queuing GET request: ${url}`
      );
      return this.queueRequestDuringRefresh({
        url,
        method: 'GET',
        data: params,
        config: config || {},
  ...
  public post<T = any>(
    ...
    if (this.isRefreshing && !this.isRefreshTokenRequest(url)) {
      this.logger.info(
        `Token refresh in progress, queuing POST request: ${url}`
      );
      return this.queueRequestDuringRefresh({
        url,
        method: 'POST',
  ...
  // 在Token刷新期间将请求加入队列
  private queueRequestDuringRefresh(params: {
    url: string;
    method: string;
    data: any;
    config: Partial<AxiosRequestConfig>;
    ...
    return new Promise((resolve, reject) => {
      ...
      this.logger.info(
        `Queuing request during token refresh: ${method?.toUpperCase?.()} ${url}`
      );

      // 将请求加入待处理队列
      this.pendingRequests.push({ url, method, data, config, resolve, reject });
      this.logger.info(
        `Request queued during refresh, current queue size: ${this.pendingRequests.length}`
      );
    });
  }
  ...
  private handleTokenExpired(config: AxiosRequestConfig): Promise<any> {
    return new Promise((resolve, reject) => {
      this.logger.warn(
        `Token expired detected for ${config.method?.toUpperCase?.()} ${config.url}`
      );

      // 将请求加入待处理队列
      this.pendingRequests.push({ config, resolve, reject });
      this.logger.info(
        `Request queued due to token expiry, current queue size: ${this.pendingRequests.length}`
      );
      ...
      if (!this.isRefreshing) {
        ...
        if (timeSinceLastRefresh < this.refreshCooldown) {
          this.logger.error(
            `Token refresh too frequent. Last refresh was ${timeSinceLastRefresh}ms ago, minimum interval is ${this.refreshCooldown}ms`
          );
        ...
        this.logger.info(
          'Token refresh already in progress, request added to queue'
        );
      }
    });
  }
  ...
  private async retryPendingRequests(): Promise<void> {
    ...
    this.logger.info(
      `Starting to retry ${requests.length} pending requests in parallel`
    );

    // 并行处理所有待处理的请求
    const retryPromises = requests.map(
      async ({ url, method, data, config, resolve, reject }, index) => {
        const requestId = `${config.method?.toUpperCase?.()} ${config.url}`;
        try {
          this.logger.info(
            `[${index + 1}/${requests.length}] Retrying request: ${requestId}`
          );
          ...
          this.logger.error(
            `[${index + 1}/${requests.length}] Request retry failed: ${requestId}`,
            error
          );
          reject(error);
        }
    ...
    this.logger.info(
      `Retry completed: ${successful} successful, ${failed} failed out of ${requests.length} requests`
    );
  }
  ...
  private clearPendingRequests(error: Error): void {
    ...
    this.logger.warn(
      `Clearing ${requests.length} pending requests due to: ${error.message}`
    );
    ...
    requests.forEach(({ config, reject }, index) => {
      const requestId = `${config.method?.toUpperCase?.()} ${config.url}`;
      this.logger.warn(
        `[${index + 1}/${requests.length}] Rejecting request: ${requestId}`
      );
      reject(error);
    });

    this.logger.info(
      `Successfully cleared ${requests.length} pending requests`
    );
  }
  ...
  private async asyncRefreshToken(): Promise<void> {
    ...
    try {
      ...
      this.logger.info(
        'Processing pending requests after successful token refresh...'
      );
      // 重新发送待处理的请求
      ...
      this.logger.warn(
        'All pending requests have been rejected due to token refresh failure'
      );
    } finally {
      this.logger.info(
        'Token refresh process ended, isRefreshing reset to false'
      );
    }
  }
